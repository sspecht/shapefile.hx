...
class FarStream extends URLStream {
	public var version(getVersion, never) : Point;
	public var loadedHeader(getLoadedHeader, never) : Bool;
	public var loadedTable(getLoadedTable, never) : Bool;
	public var loaded(getLoaded, never) : Bool;
	public var indices(getIndices, never) : Array<Dynamic>;
	public var count(getCount, never) : UInt;
	...
	function onProgress(e : ProgressEvent) : Void {
		...
		if(!gotHeader && e.bytesLoaded >= 9) readHeader(e);
		if(gotHeader && !gotTable && bytesAvailable >= tableSize) readTable(e);
		if(gotTable && e.bytesLoaded > tableSize) readFileData(e);
	}

	function readHeader(e : ProgressEvent) : Void {
		// check header:
		if(!(readByte() == 0x46 && readByte() == 0x41 && readByte() == 0x52))  {
			dispatchEvent(new IOErrorEvent(IOErrorEvent.IO_ERROR, false, false, "File is not FAR formatted"));
			close();
			return;
		}
		vMajor = readByte();
		vMinor = readByte();
		if(vMajor > VMAJOR)  {
			dispatchEvent(new IOErrorEvent(IOErrorEvent.IO_ERROR, false, false, "Unsupported archive version (v." + vMajor + "." + vMinor + ")"));
			close();
			return;
		}
	...
	function readTable(e : ProgressEvent) : Void {
		var i : FarItem;
		var evt : FarEvent;
		var l : Int;
		...
		while(0 != (l = readUnsignedInt())) {
			if(!bytesAvailable)  {
				dispatchEvent(new IOErrorEvent(IOErrorEvent.IO_ERROR, false, false, "Corrupted FAR table"));
				close();
				return;
			}
			...
			var compressed : Bool = readByte() != 0;
			if((i = dict[index]) == null)  {
				// create a new item:
	...
	function readFileData(e : ProgressEvent) : Void {
		var i : FarItem;
		var c : Int;
		var evt : FarEvent;
		var j : UInt = 0;
		while(j < table.length) {
			...
			if(c >= i.offset && !i.loaded && i.offset != -1 && i.size != -1)  {
				// read data to item:
				var count : Int = Math.min(i.size - i.data.length, bytesAvailable);
				...
				if(bytesAvailable) readFileData(e);
			}
			j++;
		}
	}
	 ...
	 */
	static public var VMAJOR : Int = 0;
	/**
	 * Highest FAR archive version this version of the API
	 * is capable of reading (minor).
	 */
	static public var VMINOR : Int = 1;
	/**
	 ...
	 * @return 
	 */
	public function getVersion() : Point {
		if(tableSize == 0) throw new Error("FAR archive header not loaded");
	 ...
	 * @see #loadedTable
	 */
	public function getLoadedHeader() : Bool {
		return gotHeader;
	}
	 ...
	 * @see #loadedHeader
	 */
	public function getLoadedTable() : Bool {
		return gotTable;
	}
	 ...
	 * 
	 */
	public function getIndices() : Array<Dynamic> {
		if(table == null) throw new Error("FAR archive table not loaded");
		var r : Array<Dynamic> = [];
		var i : UInt = 0;
	 ...
	 */
	public function exists(index : String) : Bool {
	...
	public function item(index : String) : FarItem {
		var r : FarItem;
		if((r = dict[index]) == null)  {
			if(loaded) throw (new Error("Item " + index + " is not in the currently loaded archive file"));
	...
	public function itemAt(index : UInt) : FarItem {
		if(index >= table.length) throw new Error("FAR item index out of bounds");
		return table[index];
	}
